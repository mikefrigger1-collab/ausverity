...
export default async function FirmReviewsPage() {
  ...
  const approvedReviews = firm.reviews.filter(r => r.status === 'APPROVED')
  const pendingReviews = firm.reviews.filter(r => r.status === 'PENDING')
  const reviewsWithResponse = firm.reviews.filter(r => r.response)
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-slate-900">Firm Reviews</h1>
        <p className="text-slate-600 mt-2">
          View and respond to client reviews for {firm.name}
        </p>
      </div>
                    ...
                    {review.response ? (
                      ...
                      <div className="flex justify-end">
                        <Button asChild size="sm" variant="outline">
                          <Link href={`/firm/reviews/${review.id}/respond`}>
